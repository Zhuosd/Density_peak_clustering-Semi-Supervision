...
function [Accuracy_train,predict_label_train,Accuracy_test,predict_label_test]=SSC_DensityPeaks_KNN(train,label_train,test,label_test,t,K,nneigh)
    format short;
    ...
    while(length(struct)>0)
        ...
        % [predict_label,nn_index,accuracy] = KNN_(K,data_TR, lable_TR, data, label_data)

        [accuracy, predict_label]=KNN_classifier(data_TR, lable_TR, data, label_data, K); % accuracy
        fprintf('label_data=%d\n',label_data);
